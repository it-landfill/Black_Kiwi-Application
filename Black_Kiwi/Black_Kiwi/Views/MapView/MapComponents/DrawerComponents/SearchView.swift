                    ...
                    Task {
                        if let curLocs = locationManager.getCoordinatesWithNoise(dummyUpdateModel: appSettings.locationPrivacyModel) {
							if let trueLoc = locationManager.getCoordinates() {
                                let poiList = await DataManager.getReccomendations(apiToken: appSettings.apiToken, dummyPositions: curLocs, truePosition: trueLoc, category: poiCat, minRank: minRank, limit: nil)
